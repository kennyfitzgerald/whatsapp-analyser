...
class WhatsApp:
    ...
    def _remap_members(self):
        ...
        for timestamp, change in number_changes.iteritems():
            ...
            entry_exit_after = self.entries_exits[self.entries_exits.index > timestamp]
            entry_exit_after = entry_exit_after.reindex(index=entry_exit_after.index[::-1])
            ...
            for sender, action in entry_exit_after.itertuples(index=False):
                if action == 'joined':
